package jpark.bro.data.repository
...
import jpark.bro.data.model.UserData
...
//    private val tokenManager: TokenManager
    ...
    // 인증 상태
    private val _authState = MutableStateFlow<ApiResult<UserData>>(ApiResult.Loading)
    override val authState: Flow<ApiResult<User>> = _authState.map { result ->
        when (result) {
...
//    override val isAuthenticated: Flow<Boolean> = tokenManager.isTokenValid
    ...
    // Google 로그인 시작
    override suspend fun signInWithGoogle(): ApiResult<User> {
...
//                }
    ...
    override suspend fun signInWithKakao(activityProvider: ActivityProvider) {
        _authState.value = ApiResult.Loading
        ...
        val activity = activityProvider.getActivity() as Activity
...
//            tokenManager.clearTokens()
    ...
    // 로그인 결과 처리
    private fun handleSignInResponse(response: androidx.credentials.GetCredentialResponse): ApiResult<UserData> {
        return try {
            // Google ID 토큰 파싱
            ...
            // 상태 업데이트
            val success = ApiResult.Success(userData)
