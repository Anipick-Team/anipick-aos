...
import com.jparkbro.info.series.components.SkeletonScreen
    ...
    when (state.uiState) {
        UiState.Loading -> {
            SkeletonScreen(
                ...
                onAction = { action ->
                    when (action) {
                        InfoSeriesAction.NavigateBack -> onNavigateBack()
                    }
                }
            )
        }
        ...
        UiState.Success -> {
            InfoSeriesScreen(
                ...
                onAction = { action ->
                    when (action) {
                        InfoSeriesAction.NavigateBack -> onNavigateBack()
                        is InfoSeriesAction.NavigateToInfoAnime -> onNavigateToInfoAnime(action.animeId)
                    }
        ...
        snapshotFlow { listState.layoutInfo }
            ...
            .collect { shouldLoadMore ->
                if (shouldLoadMore && !state.isLoading && state.animes.isNotEmpty() && state.hasMoreData) {
                    onAction(InfoSeriesAction.LoadMoreAnime)
                }
            }
    }
                ...
                LazyVerticalGrid(
                    ...
                    item(span = { GridItemSpan(gridInfo.columns) }) {
                        Column(
                            ...
                            Banner(state)
                            Text(
                                text = stringResource(R.string.info_series_total_count, state.totalCount),
                    ...
                    items(state.animes) { anime ->
                        APAnimeCard(
                            ...
                            onClick = { onAction(InfoSeriesAction.NavigateToInfoAnime(anime.animeId ?: -1)) }
                        )
                    }
